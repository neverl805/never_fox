...
use nss::{
    ...
    TLS_GRP_EC_X25519, TLS_GRP_FFDHE_2048, TLS_GRP_FFDHE_3072, TLS_GRP_KEM_MLKEM768X25519,
    TLS_VERSION_1_3, ZeroRttChecker, hkdf, hp,
    random,
};
...
impl Crypto {
    pub fn new(
        ...
        if let Agent::Client(c) = &mut agent {
            // Firefox-152 fingerprint alignment: Firefox does NOT GREASE its
            // TLS ClientHello (unlike Chrome). neqo greases by default; disable it.
            c.set_option(nss::Opt::Grease, false)?;
            // Firefox advertises signed_certificate_timestamp (extension 18).
            c.set_option(nss::Opt::SignedCertificateTimestamps, true)?;
            // Firefox uses a FIXED ClientHello extension order; NSS permutes by
            // default here. Disable permutation so the order is stable & FF-like.
            c.set_option(nss::Opt::EnableChExtensionPermutation, false)?;
            // Firefox advertises delegated_credentials (extension 34).
            c.set_option(nss::Opt::DelegatedCredentials, true)?;
            // Firefox signature_algorithms list, in Firefox's exact order.
            c.set_signature_schemes(&[
                0x0403, 0x0503, 0x0603, 0x0804, 0x0805, 0x0806, 0x0401, 0x0501, 0x0601,
                0x0203, 0x0201,
            ])?;
            // Firefox sends a GREASE ECH extension (65037).
            c.enable_ech(&[] as &[u8])?;
            // Firefox advertises compress_certificate (27): zlib, brotli, zstd.
            c.set_certificate_compression::<nss::cert_compress::Zlib>()?;
            c.set_certificate_compression::<nss::cert_compress::Brotli>()?;
            c.set_certificate_compression::<nss::cert_compress::Zstd>()?;
